...
SCIP_DECL_SEPAEXECLP(LiftedKnapsackSepa::scip_execlp) {
  ...
  *result = SCIP_DIDNOTFIND;
  ...
  for (auto row : knapsack_constraints.value()) {
     ...
     * a[0]*x[i_0] + a[1]*x[i_1] + ... + a[k]*x[i_k] <= b
    ...
    for(int i = 0; i < n_k; i++){
      if(incumbent_solution[i] == 1){
        items_taken.push_back(i);
        accumulated_weight += row_coeffs[i];
      }
      else{
        list.push_back(std::make_pair(row_coeffs[i], i));
      }
    }
    ...
    int i = 0;
    while(accumulated_weight < rhs + 1){
      auto [ratio, index] = list[i];
      accumulated_weight += ratio;
